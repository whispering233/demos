...
function change(hour, min, sec, time_gap=25) {
    ...
    // let s_c = sec * 6 * (1/0.96);

    const total_sec = 24 * 60 * 60 * (time_gap / 24);
    let now_sec = (hour * 60 * 60 + min * 60 + sec) * (time_gap / 24);

    let h_c = (now_sec / total_sec) * 360;
    let m_c = ((now_sec / 60) % 60) * 6
    let s_c = Math.round((now_sec) % 60) * 6;
    // let s_c = (now_sec) % 60 * 6;
